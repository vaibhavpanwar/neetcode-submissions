...
//SC -->O(M+N) O(M+N+N) for Trie (m is total characters in string of dictionary) N is the length of string (recusrive stack) and N for DP array
// TC-> O(M+ N^2)--> M for trie build and N for total string length N^2 because checking for Length-index times for each check state
